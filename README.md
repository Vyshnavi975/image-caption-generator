...
1. **Real captioning (recommended)** — if you set an `OPENAI_API_KEY`
   ...
   ball across a grassy lawn."). An alternative vision backend is also
   supported — see Setup below.
...
- Automatic backend selection: GPT-4o vision → local heuristic demo mode
  (or an alternative vision backend — see Setup), based on which API key
  (if any) is set.
...
│   ├── llm.py                  # real captioning via the OpenAI vision API (or an alternative)
...
pip install anthropic      # only if using the alternative backend (see below)
...
Anthropic Claude is also supported as an alternative backend, used if
...
If you set `OPENAI_API_KEY` (or the alternative key from Setup above) and
re-run the same command, `Backend:` will read `openai` (or the alternative
backend's name) and `Caption:` will be a real, content-aware description
...
- The real API backends make network calls and consume API credits/quota;
  they require `pip install openai` (or the alternative backend's SDK —
  see Setup) — not required for demo mode.
