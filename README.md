...
1. **Real captioning (recommended)** — if you set an `ANTHROPIC_API_KEY`
   (Claude) or `OPENAI_API_KEY` (GPT-4o) environment variable, the image is
   sent to that vision-capable model and you get a genuine, content-aware
   caption ("A golden retriever puppy chasing a red ball across a grassy
   lawn.").
...
- Automatic backend selection: Claude vision → GPT-4o vision → local
  heuristic demo mode, in that order, based on which API key (if any) is
  set.
...
│   ├── llm.py                  # real captioning via Anthropic / OpenAI vision APIs
...
With neither set, the tool automatically uses demo mode — nothing else to
configure.
...
If you set `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` and re-run the same
command, `Backend:` will read `anthropic` or `openai` and `Caption:` will be
a real, content-aware description generated by the vision model instead.
...
- The real API backends make network calls and consume API credits/quota;
  they require `pip install anthropic` and/or `pip install openai`
  respectively (not required for demo mode).
